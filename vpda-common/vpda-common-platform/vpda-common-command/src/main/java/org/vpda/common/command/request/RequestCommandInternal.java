 ...
 *
 */
final class RequestCommandInternal<T> implements Command<T> {
    private final AbstractRequestCommand<T> reqCmd;

    RequestCommandInternal(AbstractRequestCommand<T> reqCmd) {
    ...
    public T execute(CommandExecutor executor, CommandExecutionEnv env, CommandEvent event) throws Exception {
        return reqCmd.executeRequest(executor, env, event);
    }
