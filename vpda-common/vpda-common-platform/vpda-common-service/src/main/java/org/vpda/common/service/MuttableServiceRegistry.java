/**
 * View provider driven applications - java application framework for developing RIA
 * Copyright (C) 2009-2022 Roman Kitko, Slovakia
 *
 * Licensed under the GNU GENERAL PUBLIC LICENSE, Version 3.0 (the "License").
 * You may not use this file except in compliance with the License.
 * You may obtain a copy of the License at http://www.gnu.org/licenses/gpl.html
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
/**
 * 
 */
package org.vpda.common.service;

/**
 * @author kitko
 *
 */
public interface MuttableServiceRegistry extends ServiceRegistry {

    /**
     * Register service
     * 
     * @param serviceClass
     * @param <T>
     * @param service
     */
    public <T extends Service> void registerService(Class<T> serviceClass, T service);

    /**
     * Register all service from {@link ServiceRegistry} sr which are not registered
     * in this service
     * 
     * @param sr
     */
    public void registerNotRegisteredServices(ServiceRegistry sr);

    /**
     * Register factory for service
     * 
     * @param factory
     */
    public <T extends Service> void registerServiceFactory(ServiceFactory<T> factory);

    /**
     * Gets factory for service class or null if not registered
     * 
     * @param serviceClass
     * @return factory
     */
    public <T extends Service> ServiceFactory<T> getFactory(Class<T> serviceClass);

}
